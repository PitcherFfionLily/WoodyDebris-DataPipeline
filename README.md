...
Input files:
- **Data0_Raw/**: This Folder contains the eight raw .xlx files for the woody debris census for each year (2017-2024). Each file contains multiple sheets including a separate sheet for Fallen CWD, Standing CWD and Standing FWD. These raw files are inputted into the .rmd file to be formatted individually, the code does not run automated through every file in the folder as column heading and structure vary for each file. Within the code the raw file must be specified.
- **Corrections/colnamechange**: This folder contains eight .txt files with the original names of column headings in the raw data and corresponding new column headings. These files are inputted into the .rmd to be used to change the column headings of the raw data.
...
The aim of this file is to apply corrections to the formatted .txt files and merge data across multiple censuses by sample codes (code_of_piece) for each census type, generating three .csv files, one for fallen CWD, one for standing CWD and  one for standing FWD. Corrections include, fixing typos and moving data to the correct field, aligning data types, removing duplicates, removing occurrences where the sample was <200mm or had fully decomposed from CWD censuses etc. Additional codes are assigned to pieces based on descriptive terms in the notes column. Further details of corrections applied can be found the inputted corrections file (see below).


Input:
- **Data1_Formatted/** : As described previously this folder contains the formatted files in the form of .txt generated from the clean0_bci50deadwood_rawtoformatted.rmd. There is an individual .csv file for each census (Fallen CWD, Standing CWD and Standing FWD) for each year. 
- **Corrections/bciCDW40Corrections_All.csv**: Contains individual row by row corrections for data across years. This file is inputted into the .rmd to be used to apply the corrections.
  ...
  - *uniqid* column identifies the unique ID given to each row within that data file.
  - *Field* column specifies the column the error to be corrected occurred.
  - *OldValue* the existing value that needs to be changed.
...
- **Corrections/masscorrect_deadwood_bci50ha.csv**: Contains corrections to applied for individual columns across whole dataset for cases where certain errors appear multiple times across the datasets. This file is inputted into the .rmd to be used to apply the corrections.
  - *file* column specifies the formatted .txt file the correction is being applied to.
  - *colname* column specifies the column the error to be corrected occurred.
  - *OldValue* the exisiting value that needs to be changed.
  - *NewValue* the correct value to be inserted.
  - *notes* details of the cause of the error, why it must be changed.
- **Corrections/newcodesadd_bci50ha.csv**:The file contains new descriptive codes for woodydebris samples translated from recurring descriptive notes in the raw data. This file is inputted into the .rmd and adds a new code columns to the input data named coded_notes. A letter based code is pasted where it is TRUE, as specified in the newcodesadd_bci50ha.csv file.
...
**CalculationExplanation_for_deadwoodStocks&Fluxes.pdf** contains a description of the steps involved in calculating deadwood stocks and fluxes that is conducted in **density_calculation_bci50deadwood_correctedtoprocessed.rmd** and **report_bci50CWD_2017-24.html** using the clean data in Data2_Corrected.
