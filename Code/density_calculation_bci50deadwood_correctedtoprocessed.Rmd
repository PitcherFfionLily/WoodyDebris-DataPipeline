...
AGBCALCULATION<-function(indata) {
  indata$AGB.kg.dry.mass <-
        # branches == 0 means sample has ALL BRANCHES 
        # calculate AGB using environment specific tree AGB allometry function 
        # specifically equation 7 of Chave et al. 2014 doi: 10.1111/gcb.12629
    ifelse(indata$branches==0|is.na(indata$branches), 
       ...
       # branches==0 means SOME BRANCHES lost, 
       # same calculation as for all branches, except multiply by 0.875 to account for lost branches
       # following Gora et al. 2019 
    ifelse(indata$branches==1,  
       (exp(-1.803 - 0.976 * 0.0561 + 0.976 * log(indata$pendensity.kgm3) + 
              2.673 * log((indata$mean_diameter_at1.3_mm*0.01)) -
              0.0299*(log((indata$mean_diameter_at1.3_mm*0.01))^2))) * 0.875,
    #calculating agb of samples NO BRANCHES - density is multiplied by volume
