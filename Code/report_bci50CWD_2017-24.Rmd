...
add_zeros<- function(indata, subplot) {
...
subplot_fallen<-add_zeros(subplot_fallen, subplot)


year_fallen<-subplot_fallen %>%
  dplyr::group_by(yearcol) %>%
  dplyr::summarise(mean_mass.kg.m2=mean(mass.kg.m2),sd_mass.mg.ha=sd(mass.kg.m2), 
                   mean_vol.m3.m2 = mean(vol.m3.m2), sd_vol.m3.ha= sd(vol.m3.m2) )
```
...
indata<-indata %>%
  dplyr::group_by(yearcol, subplot_code, transect_section_letter) %>%
...
```{r, echo=FALSE,results='hide', message=FALSE, warning=FALSE}
ggplot(subplot_fallen_new %>% filter(!yearcol=="2009"),aes(x = log(input.mass.mgha , base = 2), group=yearcol, fill = as.factor(yearcol)))+
  geom_histogram(colour = "black", 
                 bins = 40, position="stack")+
 # facet_wrap("yearcol")+
...
# Step 1: Count the number of entries for each yearcol
...
summary_type<-merge(mean_year_branch, mean_year_trunk, by="yearcol")
summary_type<-merge(summary_type, mean_year_unknown, by ="yearcol")

summary_type<-summary_type %>%
  pivot_longer(cols=!yearcol, names_to = "type",values_to = "total_mean_mass_mg3ha")
...
```{r, echo=FALSE}
ggplot(summary_type, aes(fill=type, y=total_mean_mass_mg3ha, x=yearcol))+
geom_bar(position = "stack", stat = "identity", colour="black")+
labs(title = "Mean Total Mass of Coarse Woody Debris", 
      x = "Year", 
      y = expression(paste("Mass (Mgha"^{-1},")"))) +
scale_fill_manual(values=c("#882255", "skyblue", "#009E73" ))+
scale_x_continuous(breaks = unique(summary_type$yearcol))+# Add titles  
theme_bw()+
