...
# aggregate cross-section mass for every subplot for every yearcol
# this calculates the total mass in kg/m2 and total volume in m3/m2 
SUBPLOTCALCULATIONS<-function(indata){
indata<-indata %>%
  dplyr::group_by(yearcol, subplot_code) %>% # 160 is the cumulative length of the transect in m 
  dplyr::summarise(mass.kg.m2=((pi/(2*160))*sum(pen_crossmass.kgm)), 
                   vol.m3.m2=((pi^2/(8*160))*sum(diam_sqr_m))) %>%
  dplyr::mutate(mass.Mg.ha=mass.kg.m2*10, vol.m3.ha=vol.m3.m2*10000)%>%
  dplyr::select(-c(mass.kg.m2,vol.m3.m2))

return(indata)
}
subplot_fallen<-SUBPLOTCALCULATIONS(fallen_data)


#now fill in 0 values in cases where there was no data t
#Add zeros to include all subplot-year combinations 
...
year_fallen<-subplot_fallen %>%
  ...
  dplyr::summarise(mean_mass.Mg.ha=mean(mass.Mg.ha),sd_mass.Mg.ha=sd(mass.Mg.ha), 
                   mean_vol.m3.ha = mean(vol.m3.ha), sd_vol.m3.ha= sd(vol.m3.ha) )
```
...
flux_calculation<-function(indata){
  yearly_count <-indata %>%
dplyr::group_by(yearcol) %>%
    #calculate number of measurements a yearcol
dplyr::summarise(count_per_year = n())
  # Create a new dataframe that counts the number of new codes for each yearcol
new_codes_by_year <- indata %>%
  arrange(yearcol) %>%
dplyr::group_by(code_of_piece) %>%
dplyr::mutate(first_appearance = min(yearcol)) %>%  
  ungroup() 
#summarize new codes by yearcol
...
subplot_fallen_old<-SUBPLOTCALCULATIONS(old_raw_fallen) 
subplot_fallen_new<-SUBPLOTCALCULATIONS(new_raw_fallen)
...
```{r, include=FALSE}
  #STANDING
#for transect level average of standing volume and mass sum the piece level mass and volume for each subplot and divide by subplot area (1600m2)
SUBPLOTSUM<-function(indata) {
indata<-indata %>%
  dplyr::group_by(yearcol, subplot_code) %>%
  dplyr::summarise(mass.kg.m2=sum(AGB.kg.dry.mass)/1600, vol.m3.m2=sum(volume_m3)/1600)%>%
  dplyr::mutate(mass.Mg.ha=mass.kg.m2*10, vol.m3.ha=vol.m3.m2*10000)%>%
  dplyr::select(-c(mass.kg.m2,vol.m3.m2))
return(indata)}
#applying functions
subplot_standing<-SUBPLOTSUM(standing_data)
...
subplot_standing_old<-SUBPLOTSUM(old_raw_standing)
subplot_standing_new<-SUBPLOTSUM(new_raw_standing)
...
```{r, echo=FALSE}
#create table that summarizes the field sampling for that yearcol
...
Summary of census effort for each yearcol including census period (start and end date), total number of pieces of wood measured each yearcol for Downed and Standing (Total Pieces) and the number of new pieces of dead wood (New pieces). Initials represent the following people involved in the sample collection: R:Pablo Ramos, V:Paulino Villarreal and E is both working as a team.
...
#swapping values for 2017 as input data not really input but remaining
...
#plot meaan and confidence intervals for each yearcol
...
Annual variability in estimated downed and standing CWD stocks as Mass with 95% confidence intervals. Colored points represent the mean(CI95%) total Mass of CWD for each yearcol.
...
ggplot(summary_mean_sublot_vol, aes (x=factor(yearcol), y=vol.m3.ha*10000, fill=Type, colour = Type))+
geom_errorbar(aes(ymin=(vol.m3.ha-1.96*se)*10000, ymax=(vol.m3.ha+1.96*se)*10000), width=.1, position = position_dodge(width = 0.6))+
geom_point(size=3, shape=21, colour="black", position = position_dodge(width = 0.6))+
...
Annual variability in estimated downed and standing CWD stocks as Volume with 95% confidence intervals. Colored points represent the mean(CI95%) total volume of CWD for each yearcol.
...
Distribution of log base 2 estimated total downed CWD Mass for each subplot, coloured by yearcol. Count represents the number of subplots which have an estimated Mass of downed CWD within that range of the x-axis.
...
Distribution of log base 2 estimated inputs of downed CWD Mass for each subplot, coloured by yearcol. Count represents the number of subplots which have an estimated Mass of downed CWD within that range of the x-axis.
...
Distribution of log base 2 estimated total standing CWD Mass for each subplot, coloured by yearcol. Count represents the number of subplots which have an estimated Mass of standing CWD within that range of the x-axis.
...
Distribution of log base 2 estimated inputs of standing CWD Mass for each subplot, coloured by yearcol. Count represents the number of subplots which have an estimated Mass of downed CWD within that range of the x-axis.
...
Annual variation in estimated downed coarse woody debris (CWD) mass for each 40×40 m dynamic subplot, grouped by total mass quantile
...
SAMPLETYPECALCULATIONS <- function(indata){
  # Reassign branchfall numbers to descriptive labels of what the sample piece is where 
  ...
  # Now make subplot calculations but group by year, subplot, and brancfall type
  indata <- indata %>%
...
# add zeros but include seperation by sample type
add_zeros <- function(indata, subplot, outdata) {
  all_subplots <- unique(subplot$subplot_code)
  all_years <- unique(indata$yearcol)
...
rmarkdown::render("report_bci50CWD_2009-24v2.Rmd",
output_file = "Output/report_bci50CWD_2017-24v2.html")
